...
# load data files
restra_df = pd.read_csv(filepath_or_buffer=os.path.join(script_dir,'data','restaurants.csv'))
cust_df = pd.read_csv(filepath_or_buffer=os.path.join(script_dir,'data','customers.csv'))
menu_items_df = pd.read_csv(filepath_or_buffer=os.path.join(script_dir,'data','menu_items.csv'))

# get ids of all dfs
RESTAURANTS = restra_df['restaurant_id'].tolist()
CUSTOMERS = cust_df["customer_id"].tolist()
MENU_BY_RESTAURANT = menu_items_df.groupby(by='restaurant_id')\
...
def generate_order():
    ...
    selected_items = random.sample(population=menu_items,k=num_items)
    items = []
    total_amt = 0.0
...
def stream_to_eventhub(interval_time,max_orders):
    ...
    try:
        while True:
            ...
            event_data_batch.add(event_data=EventData(json.dumps(order)))
...
if __name__ == '__main__':
    stream_to_eventhub(interval_time=3,max_orders=None)